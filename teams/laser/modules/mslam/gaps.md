...
- **链接**：https://roborock.feishu.cn/wiki/R8HQwynmOizAgSkKfM4cxP0knde
- **首次提及**：`inbox/0413新增/回环整理优化2.0_2026-04-13-10-57-26/回环整理优化2.0.md`

---

### Q-001 窄通道重定位——地面点提取方案待验证

- **问题**：重定位用局部地图中，2.5m 范围内只保留地面点，提取方案未定——高度滤波 vs 法向量滤波哪种更稳定？
- **建议确认人**：廖炳鑫 / 负责窄通道专项的工程师
- **优先级**：高（阻塞 D-028 落地）

### Q-002 误回环检测方案选型待确认

- **问题**：连通域分析 vs KNN 距离分布比值，哪种方案更可行？两种方向均未验证
- **建议确认人**：廖炳鑫
- **优先级**：高（阻塞 P-018 关闭）

### Q-003 强回环——BBS3D 替代 ICP 可行性待验证

- **问题**：BBS3D 是否能可靠处理远距离/大误差的强回环场景，替代 ICP 方案？
- **建议确认人**：廖炳鑫
- **优先级**：中

### Q-004 回环 Pose 输出数据结构与导航接口待设计

- **问题**：回环优化结果需给导航输出，涉及建图开始信号、数据结构定义，尚未设计
- **建议确认人**：导航团队 + 廖炳鑫
- **优先级**：中
