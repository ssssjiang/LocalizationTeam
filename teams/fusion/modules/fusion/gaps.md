...
> ✓ 内容已确认：residual 限幅 + NHC 约束 + 增大 RTK 观测方差，目标 ±2.5cm；已提取到 `decisions.md D-034`（2025-11-24 讨论，2025-12-04 完成自测）
...
- **测试结论（2026-03-25，林子越）**：窄通道策略在长通道上仍有精度问题；后续需 VSLAM 在窄通道建图解决（见 Q-011）
...
### Q-007 割草中定位差：relocating 状态超时/失败分支是否已实现

- **问题**：reloc-logic.md 状态机中 `relocating` 状态未画出超时/失败→报错路径，而产品文档定义 5min 停机报错；需确认状态机代码中是否已有该分支，或仍依赖导航层兜底
- **建议确认人**：范超 / 茹毅超
- **优先级**：高（影响产品超时兜底是否生效）

### Q-008 割草模式下"主动重定位状态"的触发来源

- **问题**：接口文档描述「割草模式下状态机忽略 SLAM 发出的 RELOCATE_BAD_LOCATION 消息，但机器进入主动重定位状态」——该状态由谁触发、如何触发，文档未说明
- **建议确认人**：茹毅超 / 导航接口负责人
- **优先级**：中

### Q-009 工作阶段非桩出启动且 RTK 差时的处理方案

- **问题**：D-033 中工作模式非桩出启动 RTK 差的策略标注 TBD；需讨论：树荫下如何启动割草、定位不好时能否通过其他方式（如边界检测）识别出界
- **建议确认人**：融合+导航团队（产品+算法联合讨论）
- **优先级**：高（影响树荫/信号差场景用户能否正常使用）

### Q-010 桩外待机 / 在桩各场景的机器端 + App 端需求

- **问题**：P-016 中 7 个异常场景的机器端和 App 端需求大部分仍为空，未明确具体行为
- **已确认（2025-10-31，曹丽娜）**：
  - 搬动后：App 一直显示搬动前位置，状态显示「重定位中」
  - 关机后：App 停在关机位置，直到重定位成功或充电状态更新桩坐标
- **仍待明确**：桩外待机情况1（RTK信号变差）、情况3（开关机不确定是否搬运）；在桩情况1/2/3；工作中重定位失败的 App 端行为
- **建议确认人**：产品（@小倩）+ 导航/融合联合对齐
- **优先级**：高（直接影响用户感知的定位显示体验）

---

### Q-011 VSLAM 窄通道建图能力（解决长通道精度问题）

- **问题**：当前窄通道假固定策略（D-025）在长通道场景下精度仍不足；需要 VSLAM 在窄通道中进行建图，以提供更可靠的位姿约束
- **建议确认人**：视觉 SLAM 负责人 + 融合定位负责人
- **优先级**：中（影响窄通道/长走廊场景精度体验）
...
- **首次提及**：融合组开发进展周报
- **核心机制已知**（2026-01-15~16 群聊讨论）：跳变后倒退时间窗口拒绝 RTK；队列对比（RTK 队列 vs fusion 队列）；收尾向量夹角+长度两维度判断切回时机；fusion 队列用 odom vs VIO lost 判断；已提取到 D-037

### G-033 假固定处理4.0

- **链接**：https://roborock.feishu.cn/wiki/CON8wZyytiuvLvkwFQfcIF55nRg
- **首次提及**：融合组开发进展周报
- **背景**：RTK 失效四类场景（缓慢漂走后跳回、跳变失效后跳回、跳变失效缓慢飘回、静止跳出短暂又回）的处理策略；核心机制见 D-037
