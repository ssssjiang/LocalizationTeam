...
Image Token: MMeNbvbnMoaMOpxZ6l6ccn6wnVb
...
Image Token: H8iMbhCkvoKGyexBQ6ScUi5UnXe
...
Image Token: Fcq2bEr6JoV1q0xJHy0cvLhRnEh
...
- 关注点：
  ...
  - 回桩后地图是否会再次优化（app上地图回桩前截一张图，回桩后等一段时间再截一张对比看看）？如果优化了地图，大概花费的时间是多少？
    1. 秒级的地图保存、数据处理时间，不像是专门为了地图优化做的。
    2. 回桩后有两个步骤，地图保存（约10s），数据处理（约3s）
