...
## 2. 行业技术现状地图
   引子：3 个 capability layer（VLA / World Models / 感知层）
   ### 2.1 VLA frontier（具身决策层；08 §7 + drift staging §3-§4）
       #### 2.1.1 国际线（RT-2 / π / Helix / GR00T 四主线）
       #### 2.1.2 国内线（GraspVLA / GO-1 / UnifoLM-VLA-0）
   ### 2.2 World Models 工具链（仿真训练层；08 §8 + drift §5）
       #### 2.2.1 DeepMind Genie 系列（consumer 向）
       #### 2.2.2 NVIDIA Cosmos（physical AI / robot industry 向）
   ### 2.3 几何重建（感知层 / 与 SLAM 对接；drift §5）
       #### 2.3.1 3DGS / 4DGS（real-time render）
       #### 2.3.2 DUSt3R / VGGT（feed-forward 大模型化）
   章末 wrap-up：跨 layer × 跨厂商 cover 矩阵表（mature 度 / 工业可用性 / 国内外标签）

## 3. Phase 路线图
   引子：north star = wheel-legged biped + 1 arm；technology mature 现状决定 phase 拆分
